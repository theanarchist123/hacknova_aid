    ...
    try {
      ...
      if (result != null) {
        if (kIsWeb && result.files.single.bytes != null) {
          ...
          await _processPDFBytes(result.files.single.bytes!, result.files.single.name);
        } else if (!kIsWeb && result.files.single.path != null) {
          // For other platforms, use path
          final file = File(result.files.single.path!);
          await _processPDF(file);
        } else {
          _showErrorSnackBar('Unable to access the selected PDF file');
        }
      }
    } catch (e) {
...
''';
  ...
  Future<void> _processPDFBytes(Uint8List bytes, String fileName) async {
    ...
    try {
      String extractedText = await _extractTextFromPDFBytes(bytes);
      ...
      if (_extractedText.isNotEmpty) {
        await _generateSummary(_extractedText, fileName);
      } else {
        _showErrorSnackBar('No text found in the PDF');
      }
    } catch (e) {
